...
const QuizzesContainer = props => {
  ...
  const [fetching_quiz_data_error, set_Fetching_Quiz_Data_Error] =
    useState(false);
  ...
  const [show_quiz_submit_button, set_Show_Quiz_Submit_Button] =
    useState(false);
  ...
  }, [submitting_quiz, fetching_quiz_data, fetching_quiz_data_error]);


  // fetch quiz data from server which ans n questions
  const fetchQuizData = () => {
  ...
  // while fetching quiz data shows loading indicator
  ...
  // triggers when user onclick right arrow button or right swipe on quiz screen and 
  // this fuction shows next question option on screen
  const onNextQuestion = () => {
    set_Current_Question_Index(prevState =>
      quiz_data.questions.length === prevState + 1 ? prevState : prevState + 1,
    );
  };

  // triggers when user onclick left arrow button or left swipe on quiz screen and 
  // this fuction shows previous question option on screen
  ...
  // when click on question option it'll change the state and background color of selected option and
  // and maintain the state of already attempted or not questions and go to next question when select
  // an option with the delay of 300 ms
  const onSelectionOfQuizOption = (question, option) => {
    ...
    let temp_attempted_quiz_data = attempted_quiz_data.map(attempted_answer => {
    ...
    setTimeout(() => {
      set_Current_Question_Index(prevState =>
        quiz_data.questions.length === prevState + 1
          ? prevState
          : prevState + 1,
      );
      set_Show_Quiz_Submit_Button(
        () => quiz_data.questions.length === current_question_index + 1,
      );
    }, 300);
  };
